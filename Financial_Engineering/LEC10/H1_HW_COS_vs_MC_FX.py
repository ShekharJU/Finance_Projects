...
def ChFH1HW_FX(u,tau,gamma,Rxv,Rxrd,Rxrf,Rrdrf,Rvrd,Rvrf,lambdd,etad,lambdf,etaf,kappa,vBar,v0):
    ...
    value1 =integrate.trapezoid(np.real(f(z,u)),z).reshape(u.size,1)
    value2 =integrate.trapezoid(np.imag(f(z,u)),z).reshape(u.size,1)
    ...
    for k in range(0,len(u)):
        ...
        value1 =integrate.trapezoid(np.real(f),z)
        value2 =integrate.trapezoid(np.imag(f),z)
        int1[k]=(value1 + value2*i)
    """   
    int2 = (u**2.0 + i*u)*integrate.trapezoid(zeta(tau-z),z)
    A = int1 + int2
...
def mainCalculation():
    ...
    plt.savefig('H1_HW_COS_vs_MC_FX_option_prices.png')
    ...
    plt.savefig('H1_HW_COS_vs_MC_FX_implied_vols.png')
